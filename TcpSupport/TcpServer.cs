    ...
    public class TcpServer : Component
    {
        ...
        private void Initialise()
        {
            try
            {
                ...
                //ConfigurationItemFactory.Default.Targets.RegisterDefinition("NLog.Contrib.Targets.WebSocketServer", typeof(NLog.Contrib.Targets.WebSocketServer.WebSocketServerTarget));
        ...
        private void RunListener()
        {
            while (_mIsOpen && _mPort >= 0)
            {
                ...
                catch (ThreadInterruptedException ex)//thread is interrupted when we quit
                {
                    _log.Error("\r\nMethod Name={0}\r\nMessage={1}\r\nStackTrace={2}\r\nInnerException={3}\r\nInnerException.Message={4}",
                        MethodBase.GetCurrentMethod().Name,
                        ex.Message + "\r\nLuồng sẽ bị ngắt khi thoát phần mềm nên đừng lo",
                        ex.StackTrace,
                        ex.InnerException,
                        (ex.InnerException == null ? string.Empty : ex.InnerException.Message));
                }
